...
template <typename T>
Vector<T>::Vector(std::initializer_list<T> list) : sz{static_cast<int>(list.size())}
{
    std::cout << "Vector: standard list constructor \n";

    elems = std::unique_ptr<T[]>(new T[list.size()]);
